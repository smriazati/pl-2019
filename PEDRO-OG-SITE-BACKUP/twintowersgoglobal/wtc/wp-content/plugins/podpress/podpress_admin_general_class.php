...
*/
	class podPressAdmin_class extends podPress_class
	{
		...
		function settings_general_edit() {
			...
			echo '						<p class="message error">'.sprintf(__('<strong>Warning:</strong> It appears you are not using WordPress permalinks resp. not a none-default permalink setting. If you want to use this statistic method, you need to choose a permalink structure which is different to the default setting. Go to the <a href="%1$s">Permalink Settings of your blog</a> and change that first. Otherwise enabling this statistics feature will most likely cause downloads of media files which were added with podPress to fail.', 'podpress'), $permalinksettingsurl).'</p>'."\n";
			...
			echo '				<li>'.__('"Full+" - If you would like to know all the information "Full" gives you and additionally which users fully downloaded files, then choose Full+ which tracks completed vs partial downloads. podPress can only try to find out whether a file transfer was complete, if the file is on the same server as your blog (if it is a local file for the script). If you add more than one media file to a post (with podPress) then these files should have different file names. Full+ MAY CAUSE DOWNLOAD PROBLEMS. TEST WELL BEFORE DEPENDING ON THIS.', 'podpress').'</li>';
			echo '				<ul>'."\n";
			...
			echo '				<td>'."\n";
			echo '					<input type="checkbox" name="disabledashboardwidget" id="disabledashboardwidget" value="yes"'; if ( (isset($this->settings['disabledashboardwidget']) AND TRUE === $this->settings['disabledashboardwidget']) or (!isset($this->settings['disabledashboardwidget'])) ) { echo 'checked="checked"'; } echo ' />'."\n";
			echo '				</td>'."\n";
			echo '				<td></td>'."\n";
			echo '			</tr> '."\n";
			...
			echo '				<td>'."\n";
			echo '					<input type="radio" name="enable3rdPartyStats" id="enablePodTracStats" value="PodTrac"'; if($this->settings['enable3rdPartyStats'] == 'PodTrac') { echo 'checked="checked"'; } echo ' />'."\n";
			echo '				</td>'."\n";
			...
			echo '				<td>'."\n";
			echo '					<input type="radio" name="enable3rdPartyStats" id="enableBlubrryStats" value="Blubrry"'; if($this->settings['enable3rdPartyStats'] == 'Blubrry') { echo 'checked="checked"'; } echo ' />'."\n";
			echo '				</td>'."\n";
			...
			echo '				<td>'."\n";
			echo '					<input type="radio" name="enable3rdPartyStats" id="disable3rdPartyStats" value="No"'; if(!in_array($this->settings['enable3rdPartyStats'], array('PodTrac', 'Blubrry'))) { echo 'checked="checked"'; } echo ' />'."\n";
			echo '				</td>'."\n";
			echo '				<td>&nbsp;</td>'."\n";
			echo '			</tr> '."\n";
			...
			if(!podPress_WPVersionCheck('2.0.0')) {
				...
				echo '			</td>'."\n";
				echo '			<td>'.__('If you want the <a href="http://en.wikipedia.org/wiki/RSS_enclosure" target="_blank" title="en.Wikipedia: RSS enclosures">enclosures</a> (element of the news feeds which contain links to the media files) to always exist (so feed will show up in iTunes), then check this.', 'podpress').'</td>'."\n";
				echo '		</tr> '."\n";
			}
