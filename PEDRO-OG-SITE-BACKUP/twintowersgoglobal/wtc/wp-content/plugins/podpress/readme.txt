...
podPress 8.8.7 is a further maintenance release which is compatible with WP 2.9.x and WP 3.0.x. (Most patches for v8.8.5+ were made by [Tim Berger (ntm)](http://profiles.wordpress.org/ntm/))
...
== Changelog ==
= v8.8.7 beta 6 =
* multiple Feed Buttons and XSPF player widgets in combination with WP 2.8+
...
= v8.8.2 =
* [Ticket #1037](http://plugins.trac.wordpress.org/ticket/1037) - multiple enhacements and fixes
...
If you have ever installed a plugin, then this 
will be pretty easy.
...
= Requirements =
podPress requires at least WP 2.0.4 but it is very recommended to use at least WP 2.3. Many features like the widgets, the custom feeds or the CategoryCasting feature will work only with WP 2.3 or later WP versions.

= Included Software =
podPress v8.8.6 includes:
...
Upgrading to 8.8.5.x, 8.8.6.x, 8.8.7 or newer versions:
The way you can customize the XSPF player widget changes with 8.8.7 (different constant names more settings via the widgets settings page). 
If you have used the widgets of podPress then it will be necessary to reactivate these widgets manually after an upgrade. 
...
podPress v8.8.5+ requires at least WP 2.0.4 but it is recommended to use at least WP 2.3.
...
Since v8.8.5 podPress produces a torrent feed again and has a new feed called "enhancedpodcast". (Both feeds are ATOM feeds) The "torrent" feed contains only posts which have .torrent files attached with podPress. The 'enhancedpodcast' feed contains only posts which have .m4a or .m4v files attached with podPress. If a post has different types of media files attached then only the right media files will show up in these feeds. In other if you have a post with an .m4a podcast and you have a torrent for this file you can add these file to your post. The 'enhancedpodcast' feed will have the post with the m4a file as an enclosure while the 'torrent' feed will show the same post but with the .torrent file as the enclosure.
(If you are not using the default permalink structure then you should save the permalink settings after the update to v8.8.5.)
...
Since v8.8.5 podPress uses the [XSPF player created by Lacy Morrow](http://blog.lacymorrow.com/projects/xspf-jukebox/) which has the possibility to use customized player skins. These skin files are XML files and situated in sub folders of podPress (/podpress/players/xspf_jukebox/dynamic/skin_1.xml and /podpress/players/xspf_jukebox/dynamic_slim/skin_1.xml). podPress overwrites these files to updated the width (and height) of the widget every time the widgets settings are saved. Later versions of podPress might have more customization options for the player.
But if you want to use your own skin files then you can prevent the overwriting of the skin files by defining the constant PODPRESS_XSPF_PLAYER_USE_CUSTOM_SKINFILE_1 (to do that uncomment the line at the beginning of the podpress_xspf_config-sample.php file). In this case you need to set the width and height constants like PODPRESS_XSPF_PLAYER_HEIGHT_1, too. Also be aware of the fact that an automatic plugin upgrade will overwrite the skin_1.xml files.
...
The name of the skin file and the custom variables file are by default skin_1.xml and variables_1.txt. The "_1" in the file names (and in the names of the constants) stands for the blog ID. In the single blog mode the blog ID is 1. If you have an multiple blog installation (WP 3.+ or WPMU) the main blog ID is also 1. The skin files are automatically created for each blog when the widget settings are saved. But if you are using custom skin files or custom variables files then you have the create the files with the right blog ID in the name manually.
