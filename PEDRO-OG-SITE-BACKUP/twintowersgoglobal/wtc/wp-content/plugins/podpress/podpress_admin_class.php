...
*/
	class podPressAdmin_class extends podPress_class
	{
		...
		function post_form($entryType = 'post') {
			...
			while ($num < $this->settings['maxMediaFiles']) {
				...
				if(!empty($files) || $this->settings['enablePodangoIntegration']) {
					...
					echo '							<span id="podPressMedia_'.$num.'_URI_chrWarning" class="updated podPressMedia_URI_chrWarning"><strong>'.__('Notice:', 'podpress').'</strong> '.__('It is not recommended to use other than these characters for folder or filenames:', 'podpress').' abcdefghijklmnopqrstuvwxzy ABCDEFGHIJKLMNOPQRSTUVWXZY 0123456789 .:/_-\</span>'."\n";
			...
			if($entryType != 'page') {
				...
				echo '						<td colspan="2">'.__('Does your podcast contain explicit language or adult content? (If you choose "Yes" or "Clean" then a corresponding notice will show up in iTunes (Explicit resp. Clean). If you choose "No" then there will be special notice.)', 'podpress').'</td>'."\n";
				echo '					</tr>'."\n";
		...
		function edit_category_form($input) {
			...
			if ( 'edit' == $action ) { // show the following form only when an existing category is going to be edited. 			
			...
			echo '				</td>'."\n";
			echo '				<td>';
			echo '				'.__('<p>You can select one or more media file types and limit by this choice the number of posts which will appear in the this Feed. For instance: if you select MP4 Video then this Feed will only contain posts (of this category) with MP4 files. If a post has also files of other types then only the files of the selected type will be attached in this feed.</p><p>Hold the key [SHIFT] or [CTRL] and use the left mouse button to select more than one value.<br />Hold [CTRL] and use the left mouse button to deselect values.</p>', 'podpress');		
			echo '				</td>'."\n";
			echo '			</tr>'."\n";
			echo '		</table>'."\n";
